...
public class Index {
	...
	void setupRender() {
		...
		if (MapApp.getMapApp().isEmpty()) {
			...
			MapApp.setOptionName("8", "", "");
			MapApp.setOptionName("9", "", "");
			...
			MapApp.setOptionName("F", "", "");
			MapApp.setOptionName("G", "", "");
			MapApp.setOptionName("H", "", "");
			MapApp.setOptionName("I", "", "");
			...
			MapApp.setOptionName("B.7", messages.get("optionb7"),
					"/restaurant/configuration/configuration");
	...
	Object onAddFavorite(String option) {
		...
		try {
			userService
					.createFavorite(favorite, userSession.getUserProfileId());
	...
	void afterRender() {
		...
		for (Option o : options) {
			...
			optionJson.put("option", o.getOption());
			optionJson.put("name", o.getOptionName());
			optionJson.put("path", o.getPath());
			if (o.isParent()) {
				JSONArray subOptionsJson=new JSONArray();
				List<Option> subOptions = o.getOptions();
				for (Option so : subOptions) {
					JSONObject subOptionJson=new JSONObject();
					subOptionJson.put("option", so.getOption());
					subOptionJson.put("name", so.getOptionName());
					subOptionJson.put("path", so.getPath());
					subOptionsJson.put(subOptionJson);
				}
				optionJson.put("suboptions", subOptionsJson);
			}
			else optionJson.put("suboptions", null);
